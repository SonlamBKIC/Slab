 ...
 * Compile with:
 *
 * gcc -o slabultilities slabultilities.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
...
{
	printf("Slab Ultilities v1.0 \n"
        "By group 2 - KSTN-DTVT K59\n\n"
		"slabultilites [-ahnpvtsz] [-d debugopts] [slab-regexp]\n"
		"-T|--Totals            Show totals slabs information\n"
		"-l|--slabs             Show detailed slabs information\n"
		"-c|--clean             Clean slabcaches\n"
...
static unsigned long long my_totals_slab(void)
{
	struct slabinfo *s;
	...
	store_size(b1, total_size);store_size(b2, total_waste);
	store_size(b3, total_waste * 100 / total_used);
	printf("Total memory used: %15s   # Loss   : %15s   MRatio:%6s%%\n", b1, b2, b3);
...
static void clean_slabcaches(void){
	printf("Total slabs before cleanning :\n");
		// Get slab infomation before cleanning
		read_slab_dir();
		unsigned long long slab_bef = my_totals_slab();
		
		// Clean slabcaches
		printf("\nCleanning slab caches ...\n");
		system("sudo sync");
    	system("echo 2 | sudo tee /proc/sys/vm/drop_caches >>/dev/null 2>>/dev/null");
		printf("Done\n\n");

		// Get slab information after cleanning
		printf("Total slabs after cleanning :\n");
		read_slab_dir();
		unsigned long long slab_aft = my_totals_slab();

		char reclaimed[20];
		store_size(reclaimed, slab_bef - slab_aft);
		printf("\nMemory reclamimed : %15s\n", reclaimed);
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "aAd::Defhil1noprstcvzTSN:LXBU",
						opts, NULL)) != -1)
		switch (c) {
		case '1':
			show_single_ref = 1;
